...
export default function JsonSchemaGenerator() {
  const { input, setInput, output, error, generateSchema } = useJsonSchemaGenerator();

  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] lg:h-screen w-full">
      <SEO title="JSON Schema Generator" description="Generate JSON Schema from JSON." />
      <ToolHeader 
        title="JSON Schema Generator" 
        description="Automatically infer JSON Schema from your data."
        actions={
          <Button onClick={generateSchema} className="gap-2 bg-indigo-600 hover:bg-indigo-700 shadow-sm shadow-indigo-200 text-white border-transparent">
      ...
      <div className="flex-1 p-6 pt-2 flex flex-col gap-4 overflow-hidden min-h-0">
        ...
        <div className="flex flex-col h-full gap-3 overflow-hidden min-h-0">
          <div className="font-bold text-[11px] text-slate-400 uppercase tracking-widest shrink-0">Generated Schema</div>
          <div className="flex-1 flex flex-col bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden min-h-0">
