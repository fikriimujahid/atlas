...
export default function JsonToCsv() {
  const { input, setInput, output, error, transform } = useJsonToCsv();

  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] lg:h-screen w-full">
      <SEO title="JSON to CSV Converter" description="Convert JSON arrays into CSV." />
      <ToolHeader 
        title="JSON to CSV" 
        description="Convert JSON array format to comma separated values."
        actions={<Button onClick={transform} className="gap-2 bg-indigo-600 hover:bg-indigo-700 shadow-sm shadow-indigo-200 text-white border-transparent"><Wand2 className="w-4 h-4" /> Convert</Button>}
      />
      <div className="flex-1 p-6 pt-2 flex flex-col gap-4 overflow-hidden min-h-0">
        ...
        <div className="flex flex-col h-full gap-3 overflow-hidden min-h-0">
          <div className="font-bold text-[11px] text-slate-400 uppercase tracking-widest shrink-0">CSV Output</div>
          <div className="flex-1 flex flex-col bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden min-h-0">
            <JsonEditor value={output} language="csv" readOnly />
          </div>
        </div>
        </div>
      </div>
